...
<template>
  <div class="bg-[#4727c6] text-[#32d52f] py-18 px-5 w-full text-center mb-7 shadow-md h-auto flex flex-col items-center justify-center">
    ...
    <div class="perpage-box mt-4 mx-5 w-full md:w-1/2 justify-items-center relative left-[24%] translate-x-[-24%]">
      <div class="w-full px-6 mb-5">
        <div class="flex flex-col items-center relative">
          <div class="w-full">
            <div class="my-8 p-4 bg-white flex border border-gray-300 rounded w-full md:w-1/2">
  ...
  <div class="box-border mb-11">
    <div class="grid items-center justify-items-center gap-x-2.5 h-14 text-lg md:text-xl px-8 md:px-6 md:mx-18 grid-cols-[minmax(0,_5.5fr)_repeat(4,_1.5fr)] font-medium">
  ...
  <!-- Gold Medal -->
  <div class="flex flex-col items-center mx-8"> <!-- 间隔扩大一倍 -->
    <span class="text-gold font-semibold text-lg">Gold</span> <!-- 设置统一的文字大小 -->
    <img src="../assets/gold-medal.svg" alt="gold medal" class="w-10 md:w-10 lg:w-12" />
  </div>

  <!-- Silver Medal -->
  <div class="flex flex-col items-center mx-8"> <!-- 间隔扩大一倍 -->
    <span class="text-gray-600 font-semibold text-lg">Silver</span> <!-- 设置统一的文字大小 -->
    <img src="../assets/silver-medal.svg" alt="silver medal" class="w-10 md:w-10 lg:w-12" />
  </div>

  <!-- Bronze Medal -->
  <div class="flex flex-col items-center mx-8"> <!-- 间隔扩大一倍 -->
    <span class="text-brown-600 font-semibold text-lg">Bronze</span> <!-- 设置统一的文字大小 -->
    <img src="../assets/bronze-medal.svg" alt="bronze medal" class="w-10 md:w-10 lg:w-12" />
  </div>
</div>
...
<div class="box-border flex flex-col">
  <div class="flex flex-wrap justify-center">
    <CountryRow v-for="country in countries" :key="country.id" :country="country" class="flex-1 m-2" />
  </div>
  
  <!-- Adjusting the margin for the button container to move it up -->
  <div class="flex justify-between gap-2 md:px-16 py-1 px-1 mt-auto"> <!-- 设置 mt-[-50px] 来向上移动50像素 -->
    
    <!-- Prev Page Button -->
    <RouterLink
      v-if="page > 1"
      :to="{ name: 'country', query: { page: page - 1, perPage: perPageInput } }"
      class="inline-flex items-center border border-[#b9e34f] px-3 py-1.5 rounded-md text-[#3dea60] hover:bg-[#972c70] bg-red-500"
    >
    ...
    <RouterLink
      v-if="hasNextPage"
      :to="{ name: 'country', query: { page: page + 1, perPage: perPageInput } }"
      class="inline-flex items-center border border-[#4274a3] px-3 py-1.5 rounded-md text-[#cad530] hover:bg-[#45e15d] bg-[#e7403d] ml-auto"
    >
