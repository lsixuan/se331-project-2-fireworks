...
<template>
  <div v-if="country" class="bg-[#f2efef] min-h-screen">
    <div>
      <nav
        class="flex items-center justify-between relative bg-[#e5373a] shadow-md p-5 pb-3 md:p-10"
      >
        <!-- Left Section: Button -->
        <div>
          <router-link
            :to="{ name: 'country' }"
            class="text-[#f1cf60] text-4xl rounded font-black tracking-wider transition duration-150 ease-in-out hover:text-[#ecc957d0] flex items-center space-x-5"
          >
        ...
        <div class="flex space-x-2 md:space-x-5">
          ...
          <router-link
            ...
            :class="[
              'px-3 py-2 md:px-4 md:py-2 text-lg md:text-xl font-semibold rounded-md transition duration-150 ease-in-out focus:outline-none hover:bg-[#437dce] hover:text-white',
              route.name === 'detail-view'
                ? 'bg-[#3dcc35] text-white'
                : 'bg-[#FEFCFB] text-[#d1d641]'
            ]"
          >
          Comment
          </router-link>
          ...
          <router-link
            ...
            :class="[
              'px-2 py-2 md:px-4 md:py-2 text-lg md:text-xl font-semibold rounded-md transition duration-150 ease-in-out focus:outline-none hover:bg-[#dd41c8] hover:text-white',
    ...
    <div class="comment flex justify-center items-center mx-auto my-10 max-w-screen-lg">
      ...
      >
        <!-- Comment Form -->
        <form
          class="comment-form w-full space-y-4 px-6 py-5 bg-[#dc2d2d] rounded-lg shadow-lg"
          @submit="submitComment"
        >
          <h4 class="text-2xl font-bold mb-2 text-gray-800">Edit information!</h4>
          <textarea
            name="comment"
            placeholder="Leave a message of support..."
            class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#31aa2f]"
            rows="4"
          ></textarea>
          <button
            type="submit"
            class="w-full py-2 bg-[#34ddda] text-white rounded-lg hover:bg-[#38b73c] transition duration-200"
          >
            Comment
          </button>
        </form>
        ...
        <div class="comment-list w-full space-y-4 px-6 py-6 bg-[#421b70] rounded-lg shadow-lg">
          <div class="w-full h-full bg-white p-4 rounded-lg shadow-md">
            <h4 class="text-xl font-semibold text-gray-700">Message Board:</h4>
            ...
            >
              <ul class="space-y-3">
                ...
                <li
                  ...
                  :key="index"
                  class="comment-item bg-gray-100 p-3 rounded shadow-sm border-l-4 border-[#395f8f] pl-3"
                >
                  {{ comment }}
                </li>
              </ul>
